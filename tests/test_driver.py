    ...
    @mark.parametrize(
        ...
        (
            ...
            (
                ...
                {"app_id": "id"},
                (
                    {"endpoint": "http://node-1:9984", "headers": {"app_id": "id"}},
                    {"endpoint": "http://node-2:9984", "headers": {"app_id": "id"}},
                ),
            ),
            (
                (
                    {"endpoint": "node-1", "headers": {"app_id": "id"}},
                    {"endpoint": "node-2", "headers": {"app_id": "id"}},
                ),
                None,
                (
                    {"endpoint": "http://node-1:9984", "headers": {"app_id": "id"}},
...
class TestTransactionsEndpoint:
    ...
    def test_fulfill(self, driver, alice_keypair, unsigned_transaction):
        signed_transaction = driver.transactions.fulfill(unsigned_transaction, private_keys=alice_keypair.sk)
    ...
    def test_send_commit_decompose_tx_valid(
        ...
        unsigned_tx = persisted_decompose_transaction
        signed_tx = unsigned_tx.sign([alice_privkey]).to_dict()
        sent_tx = driver.transactions.send_commit(signed_tx)
        assert sent_tx == signed_tx
    ...
    def test_get_raises_type_error(self, driver):
        ...
        assert exc.value.args == ("TransactionsEndpoint.get() missing 1 required keyword-only argument: 'asset_ids'",)
...
class TestOutputsEndpoint:
    ...
    def test_get_outputs_with_spent_query_param(self, driver):
        ...
        create_tx1 = driver.transactions.fulfill(create_tx1, private_keys=carol.private_key)
        driver.transactions.send_commit(create_tx1)
        # create the second transaction for carol
        create_tx2 = create_transaction()
        create_tx2 = driver.transactions.fulfill(create_tx2, private_keys=carol.private_key)
...
class TestBlocksEndpoint:
    def test_get(self, driver, sent_persisted_random_transaction):
        block_id = driver.blocks.get(txid=sent_persisted_random_transaction["id"])
        assert block_id

    def test_retrieve(self, driver, block_with_alice_transaction):
        block = driver.blocks.retrieve(block_height=str(block_with_alice_transaction))
        assert block

    def test_retrieve_skips_unavailable_node(self, driver_multiple_nodes, block_with_alice_transaction):
