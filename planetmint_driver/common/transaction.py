...
class Transaction(object):
    ...
    def __init__(self, operation, assets, inputs=None, outputs=None, metadata=None, version=None, hash_id=None):
        ...
        if (
            ...
            and not (isinstance(assets, list) and "data" in assets[0])
        ):
            raise TypeError(
                (
            ...
            isinstance(assets, list) and reduce(lambda acc, asset: acc and "id" in asset, assets, True)
        ):
            raise TypeError(("`assets` must be a dict holding an `id` property " "for 'TRANSFER' Transactions"))
    ...
    def get_asset_id(transactions):
        ...
        asset_ids = {tx.id if tx.operation == Transaction.CREATE else tx.assets[0]["id"] for tx in transactions}

        # check that all the transasctions have the same assets id
        if len(asset_ids) > 1:
            raise AssetIdMismatch(("All inputs of all transactions passed" " need to have the same assets id"))
    ...
    def from_dict(cls, tx):
        ...
        return cls(tx["operation"], tx["assets"], inputs, outputs, tx["metadata"], tx["version"], hash_id=tx["id"])
