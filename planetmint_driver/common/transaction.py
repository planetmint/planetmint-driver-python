...
class Transaction(object):
    ...
    def __init__(
        ...
        if operation == Transaction.CREATE and asset is not None and not (isinstance(asset, dict) and "data" in asset):
            raise TypeError(
                "`asset` must be None or a dict holding a `data` "
                " property instance for '{}' "
                "Transactions".format(operation)
            )
        elif operation == Transaction.TRANSFER and not (isinstance(asset, dict) and "id" in asset):
            raise TypeError("`asset` must be a dict holding an `id` property " "for {} Transactions".format(operation))
    ...
    def get_asset_id(transactions):
        ...
        asset_ids = {tx.id if tx.operation == Transaction.CREATE else tx.asset["id"] for tx in transactions}
